...
public class UpdateNotification
{
    ...
    private final ModContainer container = Loader.instance().activeModContainer();
        ...
        new Thread("TofuCraft update check")
        {
            ...
            public void run()
            {
                try
                {
                    ...
                    {
                        ModLog.log(Level.WARNING, e, "Failed to receive update info.");
                        return;
                    }
                    ...
                    {
                        ModLog.log(Level.WARNING, e, "Malformed update info.");
                        return;
                    }
    ...
    public void notifyUpdate(ICommandSender sender, Side side)
    {
        if (updateInfo != null)
        {
            if (side == Side.SERVER)
            {
                sender.sendChatToPlayer(new ChatMessageComponent().addFormatted(
                        "tofucraft.update.server", updateInfo.version, updateInfo.downloadUrl));
            }
            else
            {
                sender.sendChatToPlayer(new ChatMessageComponent().addFormatted(
                        "tofucraft.update.client", updateInfo.version, updateInfo.downloadUrl));
            }
