...
{
    public static final Block frameBlock = TcBlock.tofuGrilled;
    public static final Block portalBlock = TcBlock.tofuPortal;
...
label1:
            ...
            for(int i5 = -1; i5 <= 1; i5++)
            {
                for(int i6 = 0; i6 < 3; i6++)
                {
                    for(int j7 = -1; j7 < 3; j7++)
                    {
                        ...
                        boolean flag = j7 < 0;
                        world.setBlock(k8, l9, i11, flag ? frameBlock.blockID : 0, 0, 2);
                    }
    ...
    public boolean placeInExistingPortal(Entity par1Entity, double par2, double par4, double par6, float par8)
    {
        ...
        else
        {
            for (k1 = l - short1; k1 <= l + short1; ++k1)
            {
                ...
                for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1)
                {
                    ...
                    {
                        if (this.worldServerInstance.getBlockId(k1, i2, l1) == portalBlock.blockID)
                        {
                            while (this.worldServerInstance.getBlockId(k1, i2 - 1, l1) == portalBlock.blockID)
                            {
                                --i2;
                            }
        ...
        if (d3 >= 0.0D)
        {
            ...
            if (this.worldServerInstance.getBlockId(i - 1, j, k) == portalBlock.blockID)
            {
                j2 = 2;
            }
            ...
            if (this.worldServerInstance.getBlockId(i, j, k - 1) == portalBlock.blockID)
            {
                j2 = 3;
            }
    ...
    public boolean makePortal(Entity par1Entity)
    {
            ...
            for (k2 = -1; k2 <= 1; ++k2)
            {
                for (i3 = 1; i3 < 3; ++i3)
                {
                    for (l2 = -1; l2 < 3; ++l2)
                    {
                        ...
                        i4 = j2 + (i3 - 1) * l5 - k2 * k5;
                        flag = l2 < 0;
                        this.worldServerInstance.setBlock(k3, j3, i4, flag ? frameBlock.blockID : 0, 0, 2);
                    }
                }
            }
        }
        ...
        // Place blocks
        ModLog.debug("k1=%d, k5=%d, l5=%d", k1 % 4, k5, l5);
        for (k2 = 0; k2 < 4; ++k2)
        {
            for (i3 = 0; i3 < 4; ++i3)
            {
                for (l2 = -1; l2 < 4; ++l2)
                {
                    ...
                    this.worldServerInstance.setBlock(k3, j3, i4, flag ? frameBlock.blockID : portalBlock.blockID, frameMeta, 2);
                }
            }
