...
public class EntityTofuCreeper extends EntityMob
{
    ...
    public boolean getCanSpawnHere()
    {
        ...
        {
            int[] idx = TofuCreeperSeed.instance().getSpawnId(this.worldObj, WorldType.base12Biomes.length);
            int bid1 = WorldType.base12Biomes[idx[0]].biomeID;
            int bid2 = WorldType.base12Biomes[idx[1]].biomeID;
    ...
    public void onDeath(DamageSource par1DamageSource)
    {
        ...
        {
            this.dropItem(TcItem.tofuCake.itemID, 1);
        }
    }
        ...
        for (blockY = oy; blockY <= oy + height; ++blockY)
        {
            for (blockX = ox - radius; blockX <= ox + radius; ++blockX)
            {
                ...
                for (int blockZ = oz - radius; blockZ <= oz + radius; ++blockZ)
                {
                    ...
                    if (par1World.getBlockId(blockX, blockY, blockZ) != Block.mobSpawner.blockID)
                    {
                        par1World.setBlock(blockX, blockY, blockZ, TcBlock.tofuMomen.blockID, 0, 3);
                    }
                }
            }
        }
