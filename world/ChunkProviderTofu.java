...
public class ChunkProviderTofu implements IChunkProvider
{
                    ...
                    for (int var31 = 0; var31 < 8; ++var31)
                    {
                        ...
                        for (int var42 = 0; var42 < 4; ++var42)
                        {
                            ...
                            for (int var51 = 0; var51 < 4; ++var51)
                            {
                                if ((var47 += var49) > 0.0D)
                                {
                                    par3ArrayOfByte[var43 += var44] = (byte)TcBlock.tofuTerrain.blockID;
                                }
            ...
            for (int var9 = 0; var9 < 16; ++var9)
            {
                ...
                for (int var16 = 127; var16 >= 0; --var16)
                {
                    ...
                    else
                    {
                        ...
                        else if (var18 == TcBlock.tofuTerrain.blockID)
                        {
                            if (var13 == -1)
                            {
                                if (var12 <= 0)
                                {
                                    var14 = 0;
                                    var15 = (byte)TcBlock.tofuTerrain.blockID;
                                }
    ...
    public void populate(IChunkProvider par1IChunkProvider, int par2, int par3)
    {
        ...
        if (this.rand.nextInt(4) == 0)
        {
            ...
            (new WorldGenLakes(TcBlock.soymilkStill.blockID)).generate(this.worldObj, this.rand, var12, var13, var14);
        }

        int var15;
        WorldGenMinable worldgenminable;

        worldgenminable = new WorldGenMinable(TcBlock.tofuKinu.blockID, 32, TcBlock.tofuTerrain.blockID);
        ...
        worldgenminable = new WorldGenMinable(TcBlock.oreTofuDiamond.blockID, 4, TcBlock.tofuTerrain.blockID);
        ...
        for (int i = 0; i < 64; i++)
        {
            for (int j = 0; j < 16; j++)
            {
                for (int k = 0; k < 16; k++)
                {
                    int x = var4 + j;
                    int z = var5 + k;
                    if (worldObj.getBlockId(x, i, z) == Block.mobSpawner.blockID)
                    {
    ...
    {
        TileEntityMobSpawner tile = (TileEntityMobSpawner)worldObj.getBlockTileEntity(bx, by, bz);
        if (tile != null && !tile.getSpawnerLogic().getEntityNameToSpawn().equals("TofuCreeper"))
        {
            tile.getSpawnerLogic().setMobID("TofuSlime");
            ...
            for (int x = -20; x < 20; x++)
            {
                for (int y = 0; y < 2; y++)
                {
                    ...
                    {
                        if ((x > -2 && x < 2 || z > -2 && z < 2)
                                && worldObj.getBlockId(x + bx, y + by, z + bz) == Block.web.blockID)
                        {
